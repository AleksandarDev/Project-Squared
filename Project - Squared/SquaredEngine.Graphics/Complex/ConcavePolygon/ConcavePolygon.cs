...
namespace SquaredEngine.Graphics {
	public struct ConcavePolygon : IComplex, IDrawable {
		...
		public ConcavePolygon(Vector3 position, Vector3[] points, Color[] pointColors) {
			...
			if (!isConcave) {
				IComplex convex = new ConvexPolygon(position, points, pointColors);
				...
				for (int index = 0; index < this.primitivesCount - 1; index++) {
					this.primitives[index] = new GraphicsDrawer.Triangle(concavePoints[0].Position + position,
					                                      concavePoints[index].Position + position,
					                                      concavePoints[index + 1].Position + position,
					                                      concavePoints[0].Color, concavePoints[index].Color,
					                                      concavePoints[index + 1].Color);
				}
				this.primitives[this.primitivesCount - 1] = new GraphicsDrawer.Line(concavePoints[0].Position + position,
				                                                     concavePoints[this.primitivesCount - 1].Position + position,
				                                                     concavePoints[0].Color,
				                                                     concavePoints[this.primitivesCount - 1].Color);
			}
		}
