...
namespace SquaredEngine.Graphics {
	public struct Ellipse : IComplex, IDrawable {
		...
		public Ellipse(Vector3 position, float width, float height, float rotation, Color color)
			: this(position, width, height, rotation, width + height < 25 ? 12 : (int)((width + height) / 2), color) {
		}

		public Ellipse(Vector3 position, float width, float height, float rotation, int sides, Color color) {
			...
			for (int index = 0; index < sides - 1; index++) {
				this.primitives[index] = new GraphicsDrawer.Triangle(position, position + points[index],
				                                                     position + points[index + 1], color);
			}
			this.primitives[sides - 1] = new GraphicsDrawer.Triangle(position, position + points[sides - 1], position + points[0],
			                                                         color);
